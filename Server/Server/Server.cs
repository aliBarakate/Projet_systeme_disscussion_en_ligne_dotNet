...
namespace Server
{
    public partial class Server : Form
    {
        ...
        {
            return string.Format("Erreur: {0}", msg);
        }

        private string SystemMsg(string msg)
        {
            return string.Format("Systéme: {0}", msg);
        }
            ...
            if (!exit)
            {
                startButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Serveur a démarré"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Serveur s'est arrété"));
                    }
                });
            }
        }
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    totalLabel.Text = string.Format("Total utilisateur: {0}", clientsDataGridView.Rows.Count);
                });
            }
        }
        ...
        private void Connection(MyClient obj)
        {
            if (Authorize(obj))
            {
                ...
                string msg = string.Format("{0} s'est connecté", obj.username);
                Log(SystemMsg(msg));
                ...
                msg = string.Format("{0} s'est déconnecté", tmp.username);
                Log(SystemMsg(msg));
                Send(msg, tmp.id);
            }
        }
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                {
                    error = true;
                    Log(SystemMsg("Addresse est requise"));
                }
                else
                {
                    try
                    ...
                    {
                        error = true;
                        Log(SystemMsg("Addresse est invalide"));
                    }
                }
                int port = 0;
                if (number.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Port est requis"));
                }
                else if (!int.TryParse(number, out port))
                {
                    error = true;
                    Log(SystemMsg("Port est invalide"));
                }
                else if (port < 0 || port > 65535)
                {
                    error = true;
                    Log(SystemMsg("Port invalide"));
                }
                if (username.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Nom d'utilisateur est requis"));
                }
                if (!error)
                {
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    sendTextBox.Clear();
                    Log(string.Format("{0} (Vous): {1}", usernameTextBox.Text.Trim(), msg));
